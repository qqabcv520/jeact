...
export class ModalComponent<T extends Component = any> extends Component {
  ...
  constructor(args: ModalComponentProps<Component>) {
    super(args);
  }
    ...
    const Buttons = () => {
      return this.buttons !== undefined ? (
        ...
        <div class="bgx-modal-buttons">
          <button class="btn btn-default" type="button" onclick={this.cancelClick}>
            取消
          </button>
          <button class="btn btn-primary" type="button" onclick={this.onOkClick}>
            确定
          </button>
        </div>
      );
...
export function mountModal<T extends Component>(args: MountModalArgs<T>) {
  const { name, $ = jQuery, ...restProp } = args;
  $[name] = function (contentProps: Partial<T>) {
    return Component.create<ModalComponent<T>>(
      ModalComponent,
      {
        ...restProp,
        ...contentProps,
